...
const ALL_TAGS = ["All", ...Array.from(new Set(JOB_SITES.map(s => s.tag))).sort()];
...
export default function ResourcesPage({ user, onLogout }) {
  ...
  const filtered = activeTag === "All"
    ? JOB_SITES
    : JOB_SITES.filter(s => s.tag === activeTag);
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 32 }}>
          {ALL_TAGS.map(tag => (
            <button key={tag} onClick={() => setActiveTag(tag)}
              ...
              {tag} {tag !== "All" && <span style={{ opacity: 0.6, fontSize: 11 }}>({JOB_SITES.filter(s => s.tag === tag).length})</span>}
            </button>
          ))}
        </div>
